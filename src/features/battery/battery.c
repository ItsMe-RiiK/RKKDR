...
MODULE_PARM_DESC(
    enable_health_mode, "Turn battery health mode on (value > 0) or off (value = 0) during module "
...
static acpi_status get_battery_health_control_status(struct battery_info *bat_status)
{
  union acpi_object *obj;
  acpi_status status;
  ...
  struct get_battery_health_control_status_input params = {
      .uBatteryNo = ACER_BATTERY_INDEX, .uFunctionQuery = 0x1, .uReserved = {0x0, 0x0}
  };
  struct get_battery_health_control_status_output ret;

  struct acpi_buffer input = {sizeof(struct get_battery_health_control_status_input), &params};
  ...
  {
    pr_err(
        "WMI battery status call returned a buffer of "
        "unexpected length %d\n",
        obj->buffer.length
    );
    kfree(obj);
  ...
  bat_status->health_mode = ret.uFunctionList & HEALTH_MODE ? ret.uFunctionStatus[0] > 0 : -1;
  bat_status->calibration_mode = ret.uFunctionList & CALIBRATION_MODE ? ret.uFunctionStatus[1] > 0 : -1;
...
static acpi_status set_battery_health_control(u8 function, bool function_status)
{
  ...
  struct acpi_buffer input = {sizeof(struct set_battery_health_control_input), &params};
  ...
  {
    pr_err(
        "WMI battery status set operation returned "
...
static void print_modes(const char *prefix, bool print_if_empty, bool health_mode, bool calib_mode)
{
  if (!health_mode && !calib_mode && !print_if_empty)
    return;

  pr_info(
      "%s modes: %s%s%s\n", prefix, health_mode ? "health mode" : "", health_mode && calib_mode ? ", " : "",
...
static acpi_status init_state(void)
{
  ...
  print_modes("available", print_state_if_empty, battery_status.health_mode >= 0, battery_status.calibration_mode >= 0);

  print_state_if_empty = false;
  print_modes("active", print_state_if_empty, battery_status.health_mode > 0, battery_status.calibration_mode > 0);
...
static void update_state(void)
{
  ...
  if (battery_status.calibration_mode != old_state.calibration_mode)
    pr_info("%s calibration mode\n", battery_status.calibration_mode ? "enabled" : "disabled");
...
static ssize_t health_mode_store(struct device_driver *driver, const char *buf, size_t count)
{
  bool param_val;
  int err;
...
static ssize_t calibration_mode_store(struct device_driver *driver, const char *buf, size_t count)
{
  bool param_val;
  int err;
...
static struct attribute *acer_wmi_battery_attrs[] = {
    &driver_attr_temperature.attr, &driver_attr_health_mode.attr, &driver_attr_calibration_mode.attr, NULL
};
...
static struct wmi_driver acer_wmi_battery_driver = {
    .driver = {.name = "acer-wmi-battery", .groups = acer_wmi_battery_groups},
};

static bool battery_initialized = false;

int rkkdr_battery_init(void)
{
  int ret;
  ...
  {
    acpi_status status;
    status = set_battery_health_control(HEALTH_MODE, enable_health_mode);
