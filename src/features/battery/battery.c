...
static acpi_status get_battery_information(u32 index, u32 battery, u32* result)
{
    ...
    {
        pr_err(
          "WMI battery information call returned buffer of unexpected length %u\n",
          obj->buffer.length
        );
        kfree(obj);
        return AE_ERROR;
...
static acpi_status get_battery_health_control_status(struct battery_info* bat_status)
{
    ...
    bat_status->calibration_mode =
      ret.uFunctionList & CALIBRATION_MODE ? ret.uFunctionStatus[1] > 0 : -1;
...
static void print_modes(const char* prefix, bool print_if_empty, bool health_mode, bool calib_mode)
{
    ...
    pr_info(
      ...
      health_mode && calib_mode ? ", " : "", calib_mode ? "calibration mode" : ""
    );
}
...
static acpi_status init_state(void)
{
    ...
    print_modes(
      "available", print_state_if_empty, battery_status.health_mode >= 0,
      battery_status.calibration_mode >= 0
    );

    print_state_if_empty = false;
    print_modes(
      "active", print_state_if_empty, battery_status.health_mode > 0,
      battery_status.calibration_mode > 0
    );
...
static struct attribute* acer_wmi_battery_attrs[] = {
  &driver_attr_temperature.attr, &driver_attr_health_mode.attr, &driver_attr_calibration_mode.attr,
  NULL
};
